...
- **The `twotool` agent.** `js --agent twotool` ships the mode assembled: `kernel`, `toolbox`, `shell`, nothing else. `jupyter_client`, `ipykernel`, and the already-imported `rich` are now declared dependencies; if an environment lacks the Jupyter packages the tools return one ERROR naming them instead of a traceback.
...
- **`/compact -m <model>` picks the summarizer for one manual compaction.** The flag overrides `compact.model` for that compaction alone and is leading-only, because the rest of the line is focus text. Auto-compaction still reads `compact.model`, whose default stays `same` (the active model).

### Changed

- **The `task` tool no longer writes the worker's prompt for it.** A subagent's system prompt is now exactly its own prompt file. The harness used to append a paragraph after it — "You are a fresh non-interactive worker agent", "Keep your final answer compressed: outcome, changed files if any, commands run, blockers", "Do not ask follow-up questions unless the task is impossible without operator input" — which landed later in the prompt than the agent's own delivery rule and asked for the extra lines an agent like `summarizer` forbids ("your reply is one line: the path to that file"). Recency gave the harness text the win. The caller writing the shape into `tasks`, or the agent declaring it in its own prompt, is now the only source. `agent_id` and `session_id` were already on the child's `ToolContext`, and nothing read the prompt copy of them.
- **`patch` absorbed `multi_patch`; there is now one edit tool.** `patch` keeps its scalar `old_string`/`new_string` form and also accepts an `edits` array, applied in order with each edit seeing the previous one's result. The array is atomic: a miss, an ambiguous match, or a no-op anywhere in it writes nothing and returns an error naming the offending edit by position. Passing both forms in one call is an error rather than a silent precedence rule. `multi_patch`, its description file, and its `prompts/defaultagent` selector entry are removed. The two tools had drifted. `multi_patch` was missing the `old_string == new_string` guard, so a no-op edit reported `1 edits` with an empty diff and the model believed a change had landed. That guard now runs for every edit in the list.
...
- **Server model metadata cached during login.** When logging in to an OpenAI-compatible endpoint, `/v1/models` metadata is fetched to harvest per-model served limits (`context_window`, `max_output_tokens`, `training_context_window`). These are cached alongside the model list and used by the runtime to prefer the server's advertised limits over the catalog's training-only hints. `ModelCacheMetadata`, `fetch_models_with_metadata`, and `test_login_with_metadata` added.
- **Friendly provider errors at the model_client boundary.** Raw SDK exceptions (missing API key, unknown provider id, authentication failure, connection error) are wrapped into one-line actionable messages with suggested next steps. `FriendlyProviderError` extends `ProviderNotLoggedInError` so existing error formatting passes it through cleanly.
...
- **Base URL always prompted during login.** Instead of only prompting when `login_base_url_field` is set, the prompt is always shown (prefilled with the current value, enter-to-keep). Variable-endpoint providers get a prefilled default; official providers get an empty default so enter retains the official default.
- **Runtime prefers server-cached model limits over the catalog.** Context-window and max-output resolution checks the login-time server metadata cache before falling back to models.dev. The server's advertised limits (`max_model_len`, `context_window`) reflect the actual served context, which may be much smaller than the training ceiling (`n_ctx_train`). The training ceiling is now only used as a cap on catalog values, never as a hard limit on its own.
- **`_error_text` helper for cleaner error display.** Error messages in CLI and TUI now pass `ProviderNotLoggedInError` (and `FriendlyProviderError`) through verbatim, formatting all other exceptions as `TypeName: message`, so friendly errors appear without the raw exception class prefix.
- **Progress and error output during models.dev cache refresh.** `ensure_fresh_catalog()` now prints "*** updating models.dev cache..." to stderr when a refresh fires, and wraps exceptions with a warning showing the error type and message, so the operator can tell whether the cache is stale or silently failing.
