...
Closed as already implemented by commit `d67d7d5` on `main`. Discovery scans
project and user `.agents/skills` plus project `.js/skills`, accepts only skill
directories containing `SKILL.md`, and reports shadow collisions. The final
offline suite, including the rewritten skill-discovery coverage, passed 1,291
tests.
