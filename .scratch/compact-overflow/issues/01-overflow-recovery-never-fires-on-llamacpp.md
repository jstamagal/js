...
- `is_context_overflow_error()` recognizes both `context size` and
  `exceed_context_size`; commit `83ee943` added the llama.cpp-specific match.
- The summarizer already retries an overflow by peeling the oldest half of its
  input, so manual `/compact` no longer has to fit the original oversized
  transcript in one request.
- Context-window resolution already asks a configured llama.cpp server for its
  served `n_ctx` before falling back to training/catalog metadata.

Verification replayed the exact llama.cpp error wording. Detection returned
true, manual compaction retried with the summary input reduced from eleven to
six messages, and compaction completed. The live configured server reported and
resolved its served window as 127488 tokens rather than its 262144-token
training ceiling.

The misleading `openai` provider wording is separate error-taxonomy work and is
not part of this resolved overflow behavior.
