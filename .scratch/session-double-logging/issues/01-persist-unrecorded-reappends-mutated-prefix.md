...
Closed as already implemented by commit `e9a6d47` on `main`. Persistence is now
anchored to the current turn's user-message object and an explicit
already-recorded flag, so mutations and loader healing cannot move the append
boundary backward. The regression test, interrupt-preservation tests, memory
tests, and lint gate all pass on the landed implementation.
